...
if (%obj.islocked == true){
   messageclient(%col.client, 'MsgClient',"Access Denied");
   return;
   }
%obj.issliding = 0;
%client = %col.client;
...
//door level chek //
////////////////////
if (%obj.lv == 4 && %ownerguid != %client.guid){
        messageclient(%client, 'MsgClient',"Access Denied"@%owner.namebase );
        return;
     }
else if (%obj.lv == 5 && %ownerguid != %client.guid && !%client.isadmin){
        messageclient(%client, 'MsgClient',"Access Denied");
        return;
     }
////////////////
//name of door//
...
if (%obj.canlock == true)  //if the door can lock on poweron
   %obj.islocked=true;     //lock it
if (%obj.canmove == false) //if it cant move
   return;                  //stop here
